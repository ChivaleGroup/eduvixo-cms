...
<head>
    ...
    <script nonce="<?= $e($nonce) ?>">try{const v=Math.min(200,Math.max(100,Math.round((Number(localStorage.getItem('eduvixo.accessibility.textScale'))||100)/10)*10));document.documentElement.style.setProperty('--eduvixo-text-zoom',String(v/100));document.documentElement.dataset.textScale=String(v)}catch{}</script>
    ...
    <div class="shell header-inner">
        ...
        <div class="header-actions">
            <div class="accessibility-menu" data-accessibility-menu>
                <button class="accessibility-toggle" type="button" aria-label="<?= $e($t('a11y.accessibility')) ?>" aria-haspopup="dialog" aria-expanded="false" aria-controls="accessibility-panel" data-accessibility-toggle><?= $icon('accessibility') ?></button>
                <section id="accessibility-panel" class="accessibility-panel" role="dialog" aria-labelledby="accessibility-title" data-accessibility-panel hidden>
                    <header><div><span><?= $icon('accessibility') ?></span><div><h2 id="accessibility-title"><?= $e($t('a11y.accessibility')) ?></h2><p><?= $e($t('a11y.text_size_help')) ?></p></div></div><button type="button" aria-label="<?= $e($t('a11y.close_accessibility')) ?>" data-accessibility-close>×</button></header>
                    <div class="accessibility-controls"><button type="button" data-text-decrease aria-label="<?= $e($t('a11y.decrease_text')) ?>">A-</button><output for="accessibility-scale" data-text-scale aria-live="polite">100%</output><button type="button" data-text-increase aria-label="<?= $e($t('a11y.increase_text')) ?>">A+</button><button type="button" data-text-reset><?= $e($t('a11y.reset')) ?></button></div>
                    <label for="accessibility-scale"><?= $e($t('a11y.text_size')) ?></label><input id="accessibility-scale" type="range" min="100" max="200" step="10" value="100" data-text-range>
                </section>
            </div>
...
<footer class="site-footer">
    ...
    <div class="shell footer-bottom"><div class="footer-credits"><span>© Copyright by Eduvixo &amp; <a href="https://www.ittsp.com/?IdRef=eduvixo.com" target="_blank" rel="noopener noreferrer">QUANT Software House</a>. All rights reserved.</span></div><span class="footer-identity"><?= $e($t('footer.identity')) ?></span></div>
</footer>
