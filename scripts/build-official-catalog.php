...
foreach ($catalog['packages'] as $id => $package) {
    ...
    $products[] = ['id' => $id, 'type' => $package['type'], 'name' => $package['name'], 'version' => $package['version'], 'channel' => $package['release_channel'] ?? 'stable', 'licensed' => !empty($package['license_download_enabled']), 'icon' => $package['icon'], 'copy' => $copy];
}
