...
$products=[];foreach($catalog['packages'] as $id=>$package){$metaKeys=array_values($package['meta_keys']??[]);if(!array_filter($metaKeys,static fn($path)=>str_ends_with($path,'_price')))$metaKeys[]='marketplace.free';$copy=[];foreach($languages as $locale=>$language)$copy[$locale]=['description'=>$lookup($language,$package['copy_key']),'meta'=>array_map(static fn($path)=>$lookup($language,$path),$metaKeys)];$products[]=['id'=>$id,'type'=>$package['type'],'name'=>$package['name'],'version'=>$package['version'],'channel'=>$package['release_channel']??'stable','licensed'=>!empty($package['license_download_enabled']),'icon'=>$package['icon'],'copy'=>$copy];}
