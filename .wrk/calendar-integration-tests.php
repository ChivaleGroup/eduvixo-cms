...
try {
    ...
    $telegramStart=(new DateTimeImmutable('now',new DateTimeZone('Asia/Bangkok')))->modify('+3 days');$telegramNotice=$notice;$telegramNotice['title']='Telegram reminder';$telegramNotice['description']='Detailed agenda.';$telegramNotice['timezone']='Asia/Bangkok';$telegramNotice['start_at']=$telegramStart->format('Y-m-d H:i:s');$telegramNotice['end_at']=$telegramStart->modify('+1 hour')->format('Y-m-d H:i:s');$telegramNotice['channels']=['telegram'];$telegramId=$repo->save($telegramNotice,1,null,false)['event']['id'];$db->exec('UPDATE calendar_reminders SET scheduled_at=UTC_TIMESTAMP() WHERE event_id='.(int)$telegramId);$worker->run();
    ...
    $sql($source.'/migrations/deliveries-down.sql');$assert((int)$db->query('SELECT COUNT(*) FROM calendar_deliveries')->fetchColumn()===6,'rollback preserves delivery audit');
