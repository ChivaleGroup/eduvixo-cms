...
try {
    ...
    $telegramStart=(new DateTimeImmutable('now',new DateTimeZone('Asia/Bangkok')))->modify('+3 days');$telegramNotice=$notice;$telegramNotice['title']='Telegram reminder';$telegramNotice['description']='Detailed agenda.';$telegramNotice['timezone']='Asia/Bangkok';$telegramNotice['start_at']=$telegramStart->format('Y-m-d H:i:s');$telegramNotice['end_at']=$telegramStart->modify('+1 hour')->format('Y-m-d H:i:s');$telegramNotice['channels']=['internal','browser','telegram'];$telegramId=$repo->save($telegramNotice,1,null,false)['event']['id'];$db->exec('UPDATE calendar_reminders SET scheduled_at=UTC_TIMESTAMP() WHERE event_id='.(int)$telegramId);$worker->run();
    $telegramEvent=$db->query("SELECT title,message,channels FROM notification_events WHERE subject='".(int)$telegramId."' AND channels='telegram-notifications' ORDER BY id LIMIT 1")->fetch();$assert($telegramEvent['title']==='Reminder: '.$telegramStart->format('Y-m-d H:i e'),'Telegram reminder includes local date, time and IANA time zone');$assert($telegramEvent['message']==="\nTitle: Telegram reminder\n\nDescription: Detailed agenda.\n",'Telegram reminder includes title and description with requested spacing');$assert($telegramEvent['channels']==='telegram-notifications','Telegram reminder uses only the selected provider');
    $assert((int)$db->query("SELECT COUNT(*) FROM notification_events WHERE subject='".(int)$telegramId."' AND channels='web-push'")->fetchColumn()===2,'browser and Telegram payloads remain independent per recipient');$assert((int)$db->query("SELECT COUNT(*) FROM notification_events WHERE subject='".(int)$telegramId."' AND channels LIKE '%,%'")->fetchColumn()===0,'different Calendar channels never merge payloads');
    ...
    $sql($source.'/migrations/deliveries-down.sql');$assert((int)$db->query('SELECT COUNT(*) FROM calendar_deliveries')->fetchColumn()===10,'rollback preserves delivery audit');
