...
[$status,$raw]=$request('https://www.eduvixo.com/api/marketplace/v1/official/');$assert($status===200,'official catalog endpoint online');$catalog=App\Core\OfficialCatalog::verify($raw);$assert(($catalog['core']['version']??'')==='1.0.3'&&count($catalog['products'])===13,'signed catalog is current and complete');
foreach(['en','de','zh','vi','th','lo','pl']as$locale){[$status,$page]=$request('https://www.eduvixo.com/'.$locale.'/marketplace/');$assert($status===200&&substr_count($page,'id="package-')===13,'website Marketplace anchors available: '.$locale);}
