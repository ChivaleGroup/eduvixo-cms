...
## One alert per live-chat conversation

- Base CMS `1.0.14` emits one external alert per provider, authorized recipient and conversation while the conversation is `queued`. The stable event key uses the conversation identifier instead of each message identifier.
- Existing delivery history is checked before queueing, including message-based keys produced by older releases, so an already-notified conversation does not generate another first alert after the upgrade.
- Once an operator accepts the conversation and its status becomes `assigned`, the audience gate suppresses every later external chat alert. Messages remain available normally inside the CMS.
- Isolated production-host validation passed 36 assertions, including the initial sender/message alert, suppression of a second queued message, suppression after operator acceptance and legacy durable-event deduplication.
- Signed Base CMS `1.0.14` artifacts: core package 761,519 bytes, SHA-256 `ad23b541fea090319c96b5e352ee008841cabe5af6c6c8b02071a484edc89261`; clean installer 10,407,618 bytes, SHA-256 `192bdf1f75f1d936e53b7e7413b03648a1371ba717508d055989ee6ea31a6117`. The complete release verifier passed 106 assertions and the isolated clean-install test passed 12 assertions before removing its temporary database, user and files.
- Both production installations were updated through the signed System Update lifecycle. The production postcheck passed 11 assertions, the notification worker found no pending delivery, all services remained active and no recent critical PHP-FPM error appeared.
- Server recovery point: `/root/eduvixo-backups/chat-single-alert-pre-20260902-053922`. Private source snapshot: `F:\Git\ChivaleGroup\.backups\chat-single-alert-base-cms-1.0.14-source-20260902.tar.gz`, 10,282,923 bytes, SHA-256 `ba5ff5e391ed65d9e0648e9bdf24672c0d0f36746752d65625717caa17e46438`.
