...
if (($stage['type'] ?? '') !== 'theme' || ($stage['slug'] ?? '') !== 'eduvixo' || ($stage['version'] ?? '') !== $expected) throw new RuntimeException('Unexpected theme package identity.');
$installed = $manager->install((string) $stage['token'], $actor);
$manifest = json_decode((string) file_get_contents($root . '/themes/eduvixo/theme.json'), true, 32, JSON_THROW_ON_ERROR);
if (($manifest['version'] ?? '') !== $expected) throw new RuntimeException('Theme update did not reach the expected version.');
echo json_encode(['ok'=>true,'site'=>parse_url((string)$config['base_url'],PHP_URL_HOST),'theme'=>'eduvixo','version'=>$expected,'active'=>(bool)($installed['active']??false)], JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES) . PHP_EOL;
