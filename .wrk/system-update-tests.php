...
session_start();
$candidate=rtrim((string)($argv[1]??'/root/eduvixo-deploy/core-candidate'),'/');$code=$candidate.'/cms';
...
try{
    ...
    mkdir($fixture.'/config',0700);file_put_contents($fixture.'/config/app.php','<?php return [];');file_put_contents($fixture.'/.env','CMS_SECRET_KEY="preserve-secret"');mkdir($fixture.'/themes',0755);file_put_contents($fixture.'/themes/school.txt','preserve theme');mkdir($fixture.'/storage',0700);file_put_contents($fixture.'/storage/uploads.txt','preserve uploaded data');
    ...
    $assert(count($catalog['products'])===13,'official catalog has all website products');
    foreach(['en','de','zh','vi','th','lo','pl'] as $locale)$assert(count(array_filter($catalog['products'],static fn($p)=>!empty($p['copy'][$locale]['description'])))===13,'all descriptions present: '.$locale);
    ...
    foreach(['../config/app.php','.env','themes/brand.php','plugins/tool.php','public/uploads/exploit.php','app/../config/app.php','app\\evil.php'] as $path)$assert(!App\Core\SystemUpdate::allowed($path),'protected path rejected: '.$path);
    $assert(App\Core\SystemUpdate::allowed('config/app.php')&&App\Core\SystemUpdate::allowed('database/migrations/026_email_system.sql'),'versioned configuration and e-mail migration are updateable');
    ...
    $assert(file_get_contents($fixture.'/themes/school.txt')==='preserve theme'&&file_get_contents($fixture.'/storage/uploads.txt')==='preserve uploaded data'&&file_get_contents($fixture.'/.env')==='CMS_SECRET_KEY="preserve-secret"','theme, secrets and uploads preserved');
    $assert(str_contains((string)file_get_contents($fixture.'/config/app.php'),"'mail' =>"),'versioned application configuration updated');
    ...
    $expectedMigrations=['023_demo_user_read_only.sql','024_system_notifications.sql','025_web_push.sql','026_email_system.sql'];$recorded=$db->query("SELECT name FROM migrations WHERE name LIKE '02%' ORDER BY name")->fetchAll(PDO::FETCH_COLUMN);$assert(count(array_intersect($expectedMigrations,$recorded))===4,'all additive migrations recorded: '.implode(',',$recorded));
    $assert((bool)$db->query("SHOW TABLES LIKE 'web_push_subscriptions'")->fetchColumn(),'Web Push schema is available after update');
    $assert((bool)$db->query("SHOW TABLES LIKE 'email_action_tokens'")->fetchColumn(),'secure e-mail action token schema is available after update');
