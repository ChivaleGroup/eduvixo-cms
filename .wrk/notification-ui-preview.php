...
foreach(['EduvixoTelegram','EduvixoWhatsApp']as$dir){$manifest=json_decode(file_get_contents(dirname(__DIR__).'/.plugins/'.$dir.'/source/plugin.json'),true);$fields=[];foreach($manifest['notification_integration']['fields']as$field)$fields[]=$field+['value'=>'','configured'=>false];$notificationChannels[]=['name'=>$manifest['name'],'slug'=>$manifest['slug'],'enabled'=>false,'last_error'=>null,'fields'=>$fields];}
